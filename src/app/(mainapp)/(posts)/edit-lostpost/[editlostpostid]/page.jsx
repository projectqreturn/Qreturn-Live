...
export default function LostItemForm() {
  ...
  const { editlostpostid } = useParams();
  ...
  const handleSubmit = async () => {
    try {
      ...
      const updateData = {
        ...
        date: lostDate ? lostDate.toISOString() : postData.date,
        ...
        reward: showReward,
        price: showReward ? rewardAmount : null,
      };

      console.log("Updating post with data:", updateData);
      console.log("Reward details:", { showReward, rewardAmount, price: updateData.price });

      // Send PUT request
      const res = await fetch(`/api/post/lost?id=${editlostpostid}`, {
        method: "PUT",
        headers: {
  ...
  useEffect(() => {
    const load = async () => {
      const baseUrl = "/api/post/lost";
      let newUrl = `${baseUrl}?id=${editlostpostid}`;
      ...
      try {
        ...
        if (data.post) {
          ...
          // Set reward fields
          console.log('Reward data:', { reward: post.reward, price: post.price });
          if (post.reward) {
            setShowReward(true);
            setRewardAmount(post.price || "");
            console.log('Set reward amount to:', post.price);
          }

          // Set date
          if (post.date) {
            setLostDate(new Date(post.date));
          }
  ...
  return (
    <div className="bg-gray-950 text-white min-h-screen sm:p-48 p-10 mt-48 sm:mt-10">
      <h1 className="text-2xl font-bold mb-8">Edit Your Lost item Post.</h1>
          ...
          {showReward && (
            <div>
              ...
              <input
                ...
                value={rewardAmount}
                onChange={(e) => {
                  const value = e.target.value.replace(/[^0-9.]/g, ""); // Allow digits + dot
